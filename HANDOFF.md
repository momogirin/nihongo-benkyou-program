...
**남은 작업**:
1. **파생어 세트(품사 묶음, Part 5 대비) — 착수했다가 중단됨(2026-07-15)**.
   - **확정된 설계**: 새 단어를 추가하지 않고 기존 4,059단어 중 파생어 관계가 있는 표제어만 묶는 방식(사용자 확인 완료). 스키마는 각 단어 항목에 필드 2개 추가 — `wordFamilyId`(같은 어근 그룹의 공통 ID, 그룹의 기본형 word를 그대로 씀, 예: `"decide"`), `derivationPos`(그 단어가 이 세트 안에서 담당하는 품사: 동사/명사/형용사/부사). 파생어 없는 단어는 두 필드 다 생략. 이번 라운드는 **데이터 생성까지만** 범위(화면 반영은 별도 단계, 사용자 확인 완료).
   - **진행 방식**: 전체 4,059단어(id/word/pos만 추출한 목록)를 알파벳순 4등분(a~depress / depression~knee / knife~researcher / resemble~zoo)해서 서브에이전트 4개에 병렬 위임, 각자 `data/raw/word-families-chunk-{a,b,c,d}.json`에 `[{id, wordFamilyId, derivationPos}, ...]` 매핑만 저장(원본 `data/english-vocab-*.json`은 아직 안 건드림).
   - **중단 시점 상태**: 사용자가 "이제 스탑"이라고 중단 지시 → **chunk-a만 완료됨**(`data/raw/word-families-chunk-a.json`, a~depress 구간 담당), **chunk-b/c/d는 미완성**(파일 자체가 안 만들어짐, 서브에이전트가 끝까지 못 돌았음). 이 청크 파일들은 재현 가능한 중간 산출물이라 커밋 안 함 — 다음 세션에서 이어서 진행 시 chunk-a는 재사용 가능(내용 검증은 안 한 상태이니 사용 전 확인 필요), chunk-b/c/d는 같은 방식으로 다시 돌리면 됨(전체 단어 목록에서 알파벳 구간별로 재분배).
   - **재개 시 순서**: (1) chunk-a 내용 검증 → (2) b/c/d 마저 생성 → (3) 4개 청크 병합해서 id 중복/유효성 검사 → (4) `data/english-vocab-*.json` 4개 파일에 `wordFamilyId`/`derivationPos` 필드 반영 → (5) `npm run data:build:englishVocab` 재실행 → (6) 커밋+push.
...
4. **남은 것**: `HomePage.tsx` 연동(최근기록/이어하기 카드/통계 대시보드에 영어 도메인 미포함 — 컴파일 에러는 없지만 홈 화면에서 영어 단어 활동이 안 보임), 품사 변환 빈칸형 퀴즈(Part 5 유형, 파생어 세트 데이터 필요해서 보류 — 1번 완료 후 진행), 파생어 세트 화면 반영(학습 카드에 파생어 묶음 표시 등, 1번의 데이터 작업 이후 별도 단계).
