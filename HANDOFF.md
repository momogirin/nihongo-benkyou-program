# 작업 인계 (2026-07-20 기준, 일본어 단어 예문 보완 작업 진행 중 — 월간 API 한도로 일시 중단)

다음 Claude 세션이 이어서 작업할 때 참고할 현재 상태/맥락 요약. 이 파일은 매번 최신 상태로 덮어써서 유지한다(누적 히스토리 아님 — 히스토리는 git log 참고).

## 일본어 단어(vocab-*.json) 검사 + N2/N1 예문(exampleJp/exampleKr) 보완 (2026-07-20, 진행 중 — 미완료 남음)
...
**진행 방식**: 각 급수를 340~360개 단위 청크로 나눠(N5 2개/N4 2개/N3 6개/N2 5개/N1 8개, 총 23개) 서브에이전트에 병렬 위임. 각 에이전트는 (1) word/reading/meaningKr 오류 검사·수정 (2) exampleJp/exampleKr 신규 생성(사전 대조 없는 모델 생성 콘텐츠, 기존 vocab.ts 헤더 주석의 "generated, not cross-verified" 컨벤션과 동일선상) 담당.

**⚠️ 중요 — 서브에이전트가 스스로 "위임했다"/"기다리겠다"며 미완료 종료하는 사고가 반복 발생**: 지시받은 general-purpose 서브에이전트가 실제로는 하위 위임 도구가 없는데도 마치 백그라운드에 맡긴 것처럼 응답하고 파일을 실제로 Write하지 않은 채 끝난 경우가 여럿 있었음(N3 청크2, N2 청크5 최초 시도). 완료 보고를 받으면 **반드시 실제 파일을 열어 exampleJp/exampleKr이 채워졌는지 스크립트로 검증**할 것 — 보고 텍스트만 믿지 말 것. 미완료 확인되면 SendMessage로 같은 에이전트에 "네가 직접 Read/Write로 끝까지 완료하라, 위임 도구 없다"고 재지시하면 됨(실제로 두 건 다 재지시로 해결됨).

**⚠️ 이번 세션에서 월간 API 사용 한도(monthly spend limit)에 도달** → 진행 중이던 서브에이전트 다수가 API 에러로 중단됨. **미완료로 남은 작업**:
- N2: `data/vocab-n2.json`의 **N2-1051 ~ N2-1400 구간(청크4, 350개)**에 exampleJp/exampleKr 없음.
- N1: `data/vocab-n1.json`의 **N1-341~N1-1700, N1-2041~N1-2699 구간(청크2/3/4/5/7/8, 총 2019개)**에 exampleJp/exampleKr 없음. N1-1~340(청크1), N1-1701~2040(청크6)만 완료됨.

완료된 것(N5 718개 전체·N4 668개 전체·N3 2139개 전체·N2 1398/1748·N1 680/2699)은 이미 `data/vocab-*.json`에 병합 반영하고 `npm run data:build:vocab` + `tsc --noEmit` 통과 확인함. **다음 세션에서 이어서 할 일**: 위 미완료 구간(N2 350개, N1 2019개)을 같은 방식(청크 분할 → 서브에이전트 병렬 위임 → 완료 검증 → 병합)으로 마저 채우면 됨. 청크 분할 시 스크래치 디렉토리에 남아있던 `n2-chunk-4.json`/`n1-chunk-2,3,4,5,7,8.json` 원본(예문 없는 상태)을 재사용하거나, `data/vocab-*.json`에서 해당 id 구간을 다시 잘라내면 됨.
