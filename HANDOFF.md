...
핵심 콘텐츠(한자 학습 2138자, 문법 470개, 단어 7972개) 전부 완료, 메뉴 구조 개편(한자 통합)·학습 플로우 변경(배치→나가기/이어하기)·카드 레이아웃 조밀화·로그인/클라우드 동기화(실제 동작 확인됨)·SRS 간격반복 복습·학습 카드 디자인 리프레시(배지+채도)까지 2026-07-09 하루에 전부 구현·검증·배포 완료. 같은 날 저녁, 사용자가 퇴근하며 "데이터 품질을 전체적으로 훑어서 개선"을 요청 → 아래 데이터 품질 감사 항목도 완료.
...
**결론**: 전체 데이터셋이 이미 상당히 정확함. 이번 감사에서 실제로 고친 건 2건(문법 번역 1건, 한자 훈음 1건)뿐. 예문 뜻·단어 뜻은 표본만 봤으므로 나머지(exampleKr 약 1000개, vocab 약 7500개)를 마저 전수 검토하고 싶다면 이어서 진행 가능 — 다만 지금까지의 결과를 보면 추가로 나올 오류는 매우 적을 것으로 예상됨.

## 나중에 할 수도 있는 작업 후보 (미확정 — 필요성 자체가 아직 판단 안 됨)

- 모의고사 모드 (한자/단어/문법을 급수별로 섞은 통합 타이머 테스트 — 청해·독해 지문은 콘텐츠가 없어서 제외. QuizRunner/ResultScreen/SetupScreen은 전부 한자 전용 타입이라 재사용 불가, 새 통합 러너가 필요하다는 것까지 조사하다가 데이터 품질 작업으로 우선순위가 바뀌어 보류됨. 이어서 하려면 `QuizRunner.tsx`/`vocabQuizGenerator.ts`/`grammarQuizGenerator.ts` 구조부터 다시 참고)
