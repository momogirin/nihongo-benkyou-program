...
- **학습 카드 디자인 리프레시(2026-07-09)**: 사용자가 인스타 J-POP 단어 계정 스크린샷을 보여주며 "밋밋하다"/"채도가 낮다"고 지적 → Artifact로 시안(구조 A안: 급수·부수 배지+구분선+예문 볼드 / 팔레트 B안: 새 색 추가 없이 기존 오렌지를 solid fill로 과감하게)을 먼저 보여주고 확정 후 반영. `study-top`/`study-level-badge`(오렌지 solid fill, `--color-button-fill` 재사용)/`study-radical-chip`(오렌지 틴트, `--color-selected-bg`+`--color-primary-dark` 재사용)/`study-fields-core`+`study-fields-sub`(구분선)/`study-example-reading`(볼드) 클래스 추가(`StudyPage.css`). 새 색 토큰은 하나도 안 만들고 기존 토큰만 재사용함. 한자 학습(`StudyPage.tsx`)·한자 전체보기(`KanjiListPage.tsx`)는 배지+구분선+볼드 전부 적용, 단어(`VocabPage.tsx`)·문법(`GrammarPage.tsx`)·부수(`RadicalsPage.tsx`)는 그 도메인에 자연스러운 필드가 없어서 급수(+부수 있는 경우) 배지만 추가. `--color-border`(WCAG 3:1 접근성 요건으로 이미 캘리브레이션된 값 — tokens.css 주석 참고)는 안 건드림.
