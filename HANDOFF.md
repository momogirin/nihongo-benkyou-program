...
- **단어(단어 2138→7972단어, N5~N1 전부)**: `VocabPage` — 학습/브라우즈/퀴즈, 오답노트 연동됨
- **문법(N5/N4, 144개)**: `GrammarPage` — 학습/브라우즈/퀴즈, 오답노트 연동됨. 예문에 쓰인 한자 정보를 자동으로 보여줌(`usedKanji` 헬퍼, kanjiList 재사용)
...
1. **VocabPage에도 "사용된 한자" 정보 표시** — 방금 GrammarPage에 붙인 것과 똑같은 패턴(`usedKanji` 헬퍼를 공용 lib로 빼서 재사용하면 됨). 새 데이터 필요 없음, 바로 시작 가능한 가장 쉬운 다음 작업.
