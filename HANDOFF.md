...
1. `core2`(NGSL 1001~2000)·`core3`(NGSL 2001~2809)·`toeic`(TSL 1250개) 콘텐츠도 core1과 같은 방식(서브에이전트 병렬 작성)으로 채우기 — 총 3,059단어 남음.
   - **2026-07-14, core2 시도했다가 세션 한도(rate limit)로 중단됨**: 5개 서브에이전트 중 4개는 200단어씩 결과 파일을 썼지만(1개는 아예 못 씀), 전부 "세션 한도로 중간에 끊김 + 세이프티 분류기 확인 불가, 결과를 신뢰하지 말고 직접 검증할 것"이라는 경고가 붙어서 **저장소에 반영하지 않고 폐기함**. `core1`처럼 스팟체크(품사 유효성/빈 필드/중복 단어)까지 통과한 게 아니라 그냥 버림 — 다음 세션에서 core2는 5개 청크 전부 처음부터 다시 생성할 것 (input 청크 자체는 `joined-all.json`에서 다시 뽑으면 됨, 아래 "재현 방법" 참고).
