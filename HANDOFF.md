...
## 2026-07-24 세션(후속2) — 문장배열(文の組み立て) 완료, #14 전체 완료

- **문법 문장 배열(文の組み立て) 퀴즈 완료(#14 마지막)**: 로드맵 14번의 마지막 항목이자
  형태소 분석기가 **본질적으로 필요한** 유일 항목. 규칙 휴리스틱(조사/구두점 경계)은
  커버리지 20% + 형용사 어간 절단 오분할이라 폐기하고, **사용자 승인 하에 budoux 도입**
  (구글 경량 일본어 분절기, 사전 없는 ML 모델, 순수 JS, 수십KB). **첫 신규 런타임 의존성**.
  - **설계**: grammar 예문을 budoux로 문절 분해 → 정확히 4조각으로 정규화(JLPT ★칸 유형).
    4조각은 그대로, 5조각은 인접 최단쌍 1회 병합(마지막 조각이 문장 절반 삼키지 않게),
    3↓/6↑는 제외. **항상 budoux 자연 경계만** 써서 억지 절단이 없다. 셔플된 조각을 탭해
    선택 영역에 순서대로 넣고(다시 탭하면 뺌) "확인" → 원문과 join 일치로 채점.
  - **UI**: GrammarPage 3번째 퀴즈 종류. 선택 영역은 4칸 고정 슬롯(min-height)이라 조각
    넣고 빼도 레이아웃 안 흔들림. 셔플은 정답과 반드시 다른 순열 보장. 오답 시 정답 순서
    노출 + GrammarHint(문형·해설·예문). 같은 표면형 조각 중복(「が」 2개 등)은 값이 아니라
    개수로 소거(remainingPieces useMemo).
  - **배선**: grammar 도메인이라 오답노트·SRS·퀴즈기록 **자동 연동**(기존 addGrammarWrongNotes/
    recordSrsReview('grammar') 재사용, 별도 배선 0). `GrammarSentenceQuestion`/
    `GrammarSentenceInProgressQuiz`, BackupPayload **27→28**. 정답이 selectedId가 아니라
    조각 순서(arranged)라 answers shape만 다름.
  - **검증**: 전 급수 출제 가능(N5 23·N4 27·N3 44·N2 72·N1 67=233개), 4조각 재구성 원문
    100% 일치·셔플≠정답 233/233, tsc·build 통과. budoux 파싱은 런타임 lazy(빌드타임 아님).
  - 영향 파일: `grammarQuizGenerator.ts`, `storage.ts`, `GrammarPage.tsx`, `GrammarPage.css`.
    `package.json`(budoux 추가).
- **모의고사에 용법(用法) 편입**: 12번 원칙("개별 페이지 실전 유형을 모의고사에 다 섞는다")에
  맞춰 신규 용법 퀴즈를 vocabBatches 라운드로빈 6번째로 추가. 균일 shape이라 페이지·스토리지
  변경 0. 문장배열은 순서 배열이라 4지선다 균일 shape에 안 맞아 모의고사 제외.
- **루트 중간산출물 정리**: on_targets_*.json·patch_*.json 10개 삭제(이미 kanji.json 병합됨).
- **로드맵 상태**: **#1~14 전 항목 완료**. 남은 큰 미착수 없음(문의 조립까지 끝).
  - **의도적 미개선(억지 매핑 위험)**: 활용/가나는 급수(level) 축이 없어(활용형·가나그룹 축)
    도메인×급수 약점분석에 미편입 — 억지로 "N3 약점"이라 안내하면 학습가치 낮고 데이터도 그
    축으로 안 쌓임. 최근기록/주간통계엔 이미 들어감.
  - 한자 n1a 141자 음독(表外 희귀음독, 학습가치 낮아 보류).

---

## 2026-07-24 세션(후속) — 용법(用法) 퀴즈 완료 (#14 일부)

git 대조로 미비 작업 점검 후 착수. 결과:

- **단어 용법(用法) 퀴즈 완료(#14)**: 신규 데이터 0. 文脈規定(2번, "이 문장 → 맞는 단어")의
  **역방향** = "이 단어 → 자연스러운 문장 고르기". 정답은 entry 자신의 exampleJp를
  `blankSentence`로 가린 것(4문장 모두 대상 자리를 `_____`로 가려 답이 뻔하지 않게), 오답은
  **뜻이 겹치지 않는**(정답 2개 방지) 같은 급수 단어들의 빈칸 예문. VocabPage **7번째** 퀴즈
  종류로 추가 — 유의어와 동일 배선(오답노트·SRS·이어하기), 다른 점은 **선택지가 단어가 아니라
  빈칸 문장**이고 정답은 entry.id. `VocabUsageQuestion`/`VocabUsageInProgressQuiz`,
  BackupPayload **26→27**. 결과화면은 각 문항의 원본 exampleJp를 해설로 노출.
  검증: 전 급수 출제풀 충분(N5 592·N4 562·N3 1961·N2 1480·N1 2556), 1000문항 시뮬레이션
  구조결함·뜻충돌 0, tsc·build 통과.
  영향 파일: `vocabQuizGenerator.ts`, `storage.ts`, `VocabPage.tsx`.
- **주의(이미 push된 임시 커밋)**: `5a5a9d0 "ㄹㄴㅁㅇ"`, `df0fe40 "작업도중 커밋"`이 이미
  origin/main에 올라가 있다. `5a5a9d0`의 실제 내용은 **가나 퀴즈 회차기록**(`KanaQuizHistoryEntry`)을
  최근기록·주간통계에 편입한 완결 작업(types.ts/statsSummary/storage/HomePage/KanaPage,
  BackupPayload 24→26). 코드는 정상이나 커밋 메시지만 임시 — 이미 원격에 있어 rewrite=force
  push라 손대지 않음(파괴적 작업, 사용자 판단 사안).
- **남은 로드맵**: #14 문장배열(文の組み立て)만 미착수 — 형태소 분석기 필요(별도 규모).
  활용/가나는 이제 최근기록엔 들어가나(위 임시커밋에서 가나 편입됨) 도메인×급수 정답률
  약점분석 축은 여전히 level 개념 차이로 일부 제외.

---

## 2026-07-24 세션 — 모의고사 실전화·유의어·약점분석·일관성 마감 (커밋 다수, 미push)

이 세션은 앱이 이미 성숙한 상태에서 "실전 정합성 + 일관성 + 데이터 검증"에 집중했다.
모든 변경은 tsc 통과 + 런타임 검증까지 했고, **아직 push 안 함**(사용자 지시 대기).

- **모의고사 실전 유형화(#12)**: `mockExamGenerator`가 뜻 고르기 3종만 섞던 걸, 기존 퀴즈
  생성기들을 균일 shape으로 매핑해 라운드로빈으로 섞도록 재구성 — 한자(훈독뜻/훈독/음독),
  단어(뜻/문맥빈칸/표기/자타동사/**유의어**), 문법(뜻/빈칸). 페이지·storage 변경 0(shape·id·
  domain 유지). 表記·文法빈칸·유의어는 선택지에 정답 누설 안 되게 분기.
- **단어 유의어(言い換え類義) 퀴즈(#14 일부)**: 신규 데이터 0 — 기존 meaningKr에서 유의어쌍
  도출(같은 뜻+읽기·표기 다름, 이표기 제외). VocabPage 6번째 퀴즈 종류. **정답이 entry가
  아니라 answer**. 오탐 ~5%(한국어 동음이의 電力/全力)는 규칙 분리 불가라 코드 캐비어트로 명시.
  `VocabSynonymInProgressQuiz`, BackupPayload 23→24. 모의고사에도 편입.
- **활용 드릴 오답노트 편입(#13)**: `ConjugationWrongNoteEntry`(SRS와 같은 기본형 id 단위),
  WrongNotePage 6번째 섹션(활용표 상세=ConjTable 재사용), BackupPayload 22→23.
- **활용 홈 통합**: 활용은 SRS·오답노트 있는데 홈에만 빠져 있던 걸 메움 — "지금 할 일"에 활용
  복습(SRS due), "오답 재도전"에 활용 칩. 이로써 **6개 도메인 전부 홈 통합 완성**.
- **약점 기반 복습 강화**: `getDomainLevelAccuracies`/`getWeakestDomainLevel` 추가 — 홈의 수동
  약점 텍스트를 "단어 N2 25% · 복습하러 가기 →" 실행형 카드로 교체(급수까지 콕 집음, 표본 5미만 제외).
- **한자 음독 예시 커버리지 +286자**: 방치돼 있던 onReadingOverride 4개 패치(n5n4/n2/n3/n1b,
  327항목)를 kanji.json에 병합(음독예시 42→54%). 검증: 모든 override의 on이 KANJIDIC2 실재.
  **n1a(141자)는 보류** — KANJIDIC2엔 전부 실재하나 表外 희귀음독이라 학습가치 낮고, vocab 기계
  매칭은 오탐(短음독 접두함정+청탁 관용) 많아 안전 작성 불가.
- **부수 오류 2건 정정**: 書(日→曰)·南(干→十). KANJIDIC2 전급수 대조(2138자) 결과 불일치 4건 중
  모든 체계에서 명확한 2건만. 来(人)·墨(黑)은 한국옥편 관례상 타당해 유지.
- **한자 학습카드 보강**: 전체보기에만 있던 음독별 예시·"이 한자가 쓰인 단어"를 StudyPage 학습
  카드에도(기존 lib·CSS 재사용).
- **데이터 무결성 감사 결과**: vocab/grammar/kanji 핵심 필드 결측 0, onJp는 KANJIDIC2와 100% 일치.
  kunJp 756 "불일치"는 의도된 상용 큐레이션(전체 훈독 아님, 건드리지 말 것).
- **루트에 방치돼 있던 중간산출물**(`on_targets_*.json`, `patch_*.json`)은 2026-07-24 세션(후속2)에서
  삭제 완료 — patch는 이미 data/kanji.json에 병합됐고 코드 참조도 0. 필요 시 git 히스토리에서 복원 가능.
- **남은 로드맵**: #14 용법(用法, 오답문장=지어내기 리스크)·문장배열(文の組み立て, 형태소분석기
  필요) 미착수. 활용/가나는 퀴즈기록(SimpleQuizHistoryEntry) 미기록이라 "최근 기록"·정답률 통계
  에서 빠짐(level 축이 달라 타입 확장 필요 — 보류).
